...
#' CARD_object@spatial_location
#' @param numSample Numeric, approximate number of cells in grid within the
...
#' relative positions
#'
#' @param locationOrig Data frame, spatial location data frame of the original
#' spatial resolved transcriptomics dataset, stored in the
#' CARD_object@spatial_location
...
normCoordsTrainTest <- function(locationOrig, trainInd, testInd) {
    ...
    norm_coords_test[, 1] <- (norm_coords_test[, 1] -
        locatiooFactor_x) / scaleFactor
    norm_coords_test[, 2] <- (norm_coords_test[, 2] -
        locatiooFactor_y) / scaleFactor
...
Sigma <- function(locationOrig, trainInd, testInd, OptimalPhi, ineibor) {
    ...
    WTemp <- WTemp * Nmat
    rownames(WTemp) <- rownames(norm_cordsTemp)
    colnames(WTemp) <- rownames(norm_cordsTemp)
    ##### calculate variance matrix that I need
    DTemp <- Diagonal(x = colSums(WTemp))
    In <- Diagonal(nrow(WTemp))
    W22 <- WTemp[
        (length(testInd) + 1):nrow(WTemp),
        (length(testInd) + 1):ncol(WTemp)
    ]
    SigmaTemp <- DTemp - OptimalPhi *
        WTemp
    Sigma11 <- SigmaTemp[
        seq_len(length(testInd)),
        seq_len(length(testInd))
    ]
    Sigma12 <- SigmaTemp[
        seq_len(length(testInd)),
        (length(testInd) + 1):ncol(SigmaTemp)
    ]
    Sigma21 <- SigmaTemp[
        (length(testInd) + 1):nrow(SigmaTemp),
        seq_len(length(testInd))
    ]
    Sigma22 <- SigmaTemp[
        (length(testInd) + 1):nrow(SigmaTemp),
...
#' spatial count data
#' @param Optimalb Vector, vector of the intercept for each cel type estimated
...
MVN_CV <- function(Vtrain, locationOrig, trainInd, testInd, B, Xinput_norm,
                    Optimalb, OptimalPhi, lambda, ineibor) {
    ...
    for (ict in seq_len(length(lambda))) {
        temp <- Sigma12 %*% (Vtrain[, ict] -
            Optimalb[seq_len(nrow(Vtrain)), ict])
        solver <- solve(Sigma11, temp)
        Mu <- Optimalb[seq_len(length(testInd)), ict] - solver ### q * k
...
#' locations
#'
#' @param CARD_object CARD Object with estimated cell type compositions on the
#' original spatial resolved transcriptomics data.
#' @param NumGrids Initial number of newly grided spatial locations. The final
...
#' the shape.
#'
#' @return Return CARD object with the refined cell type compositions estimated
#' for newly grided spots and the refined predicted gene expression
#' (normalized).
#'
#' @export
#' @examples
...
#' )
#'
CARD_imputation <- function(CARD_object, NumGrids, ineibor = 10,
                            exclude = NULL) {
    ...
    if (!is.null(exclude)) {
        location_usetoSample <- location[!(rownames(location) %in%
            exclude), ]
    } else {
        location_usetoSample <- location
    }
    data <- sampleGridWithin(location_usetoSample, NumGrids, concavity = 2)
    ...
    imputation <- MVN_CV(
        Vtrain, locationOrig, trainInd, testInd,
        B, Xinput_norm, Optimalb, OptimalPhi, lambda, ineibor
    )
