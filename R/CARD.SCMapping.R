...
#'
getWeightForCell <-
    function(sc_eset, ct.varname, ct.select, sample.varname, B) {
        ...
        Mean_Cell <- sapply(seq_len(ncol(count)), function(icell) {
...
#' locations, with the first and second columns represent the 2-D x-y coordinate
#' system
...
getHighresCords <- function(Cords, numCell, shape = "Square") {
    ...
    for (i in seq_len(nrow(Cords))) {
        getPointsWithinCircle <- function(ED, i, numCell, min_distance) {
            # dis = min(ED[i,-i]) min_distance = dis / 2.0
            circle <- runifdisc(numCell,
                radius = min_distance,
        ...
        getPointsWithinSquare <- function(Cords, i, numCell, min_distance) {
            minX <- min_distance
            minY <- min_distance
            rectangular_x <- runif(numCell,
                min = Cords[i, 1] - minX,
                max = Cords[i, 1] + minX
            )
            rectangular_y <- runif(numCell,
                min = Cords[i, 2] - minY,
                max = Cords[i, 2] + minY
            )
...
#' each single cell
#'
#' @param MappintSpotCellCor a mapped correlation matrix indicating the
...
#' single cell
#'
AssignSCcords <- function(MappintSpotCellCor, Cords_new, numCell,
                        sc_eset, ct.varname) {
    MapCellCords <- NULL
    for (ispot in seq_len(nrow(MappintSpotCellCor))) {
        ...
        ispot_cords <- data.frame(
            x = as.numeric(sapply(strsplit(centerspot, split = "x"), "[", 1)),
            y = as.numeric(sapply(strsplit(centerspot, split = "x"), "[", 2))
        )
        subCordsCell <- Cords_new[Cords_new$centerSPOT == centerspot, ]
        ##### calculate Euclidiean distance with this
        EDwithCenter <- rdist(
            as.matrix(subCordsCell[, 1:2]),
            as.matrix(ispot_cords)
        )
        subCordsCell$EDwithCenter <- EDwithCenter
        subCordsCell <- subCordsCell[order(subCordsCell$EDwithCenter,
            decreasing = FALSE
        ), ]
        mapCellCordsTemp <- data.frame(CorwithSpot = mapCell[seq_len(numCell)])
        mapCellCordsTemp <- cbind(mapCellCordsTemp, subCordsCell)
        mapCellCordsTemp$CT <- colData(sc_eset)[
            rownames(mapCellCordsTemp),
            ct.varname
        ]
...
#' print(scMapping)
#'
CARD_SCMapping <- function(CARD_object, shapeSpot = "Square",
                            numCell, ncore = 10) {
    ...
    Mean_Cell <- getWeightForCell(
        sc_eset, ct.varname, ct.select,
        sample.varname, B
    )
    ...
    Cords_new <- getHighresCords(Cords,
        numCell = numCell,
        shape = shapeSpot
    )
    MappintSpotCellCor <- cor(t(res_CARD), t(Mean_Cell))
    rownames(MappintSpotCellCor) <- paste0(
        Cords$x, "x",
        Cords$y
    )
    ############ high resolution coordinates
    MapCellCords <- AssignSCcords(
        MappintSpotCellCor, Cords_new,
        numCell, sc_eset, ct.varname
    )
    ########### map scRNAseq count data
    count_sc <- assays(sc_eset)$counts
    count_sc <- count_sc[, colSums(count_sc) > 0]
    count_CT <- NULL
    count_CT <- pbmclapply(seq_len(nrow(res_CARD)), function(ispot) {
        spot <- rownames(res_CARD)[ispot]
        MapCellCords_spot <- MapCellCords[MapCellCords$centerSPOT ==
            spot, ]
        df <- as(count_sc[, MapCellCords_spot$Cell], "sparseMatrix")
        colnames(df) <- paste0(
            MapCellCords_spot$Cell, ":",
            spot
        )
        colnames(df) <- paste0(
            colnames(df), ":", MapCellCords_spot$x,
            "x", MapCellCords_spot$y
        )
        df
