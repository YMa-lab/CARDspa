...
#' ct.visualize = c("Acinar_cells","Cancer_clone_A","Cancer_clone_B",
...
CARD_visualize_prop <- function(proportion,spatial_location,ct.visualize = ct.visualize,colors = c("lightblue","lightyellow","red"),NumCols, pointSize = 3.0){
    if(is.null(colors)){
	    colors = c("lightblue","lightyellow","red")
    }else{
	    colors = colors
...
#'
CARD_visualize_prop_2CT <- function(proportion,spatial_location,ct2.visualize = ct2.visualize,colors = NULL){
if(is.null(colors)){
...
CARD_visualize_pie <- function(proportion,spatial_location,colors = NULL,radius = NULL,seed = NULL){
res_CARD = as.data.frame(proportion)
...
p = suppressMessages(
    ggplot() +
    geom_scatterpie(data=data, aes(x=x, y=y,r = radius),cols=ct.select,color=NA) +
...
#' NumCols = 6)
#'
CARD_visualize_gene <- function(spatial_expression,spatial_location,gene.visualize,colors = colors,NumCols){
