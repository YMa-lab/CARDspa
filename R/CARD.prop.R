...
Theta_S_rowMean_Tmp <- wrMisc::rowGrpMeans(as.matrix(countMat[(igs:(igs+9999)),]), grp = ct_sample.id, na.rm = TRUE)
}else{
    Theta_S_rowMean_Tmp <- wrMisc::rowGrpMeans(as.matrix(countMat[igs:nrow(countMat),]), grp = ct_sample.id, na.rm = TRUE)
...
Theta_S_rowMean <- wrMisc::rowGrpMeans(as.matrix(countMat), grp = ct_sample.id, na.rm = TRUE)
}
...
#' print(CARD_obj@Proportion_CARD[1:3,1:3])
